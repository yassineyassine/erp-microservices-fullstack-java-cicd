...
helm dep up ./${cs}-${suffix}
  if [ $helmVersion -eq 1 ]; then
helm install ${cs} ./${cs}-${suffix} --replace --namespace erp1
  else
helm install --name ${cs} ./${cs}-${suffix} --replace --namespace erp1
  fi
fi
...
helm dep up ./product-${suffix}
  if [ $helmVersion -eq 1 ]; then
helm install product  ./product-${suffix} --replace --namespace erp1
  else
helm install --name product  ./product-${suffix} --replace --namespace erp1
  fi
  if [ $helmVersion -eq 1 ]; then
...
helm dep up ./store-${suffix}
  if [ $helmVersion -eq 1 ]; then
helm install store  ./store-${suffix} --replace --namespace erp1
  else
helm install --name store  ./store-${suffix} --replace --namespace erp1
  fi
